...
export class SendEmailPopupComponent implements OnInit {
    ...
    if (foundTemplate) {
      ...
      this.selectedTemplate.widgets.values.forEach((email: any) => {
        if (email.data.id=="text") {
          console.log("email.data.elementData", email.data.elementData);
          this.emailForm.patchValue({
  ...
  //  console.log(html);   
    ...
    let payload: any = {
      ...
      email_id: this.students.map(student => student.email_id).join(),
      html:this.emailForm.value.html,
      text:this.emailForm.value.text
    }
    if (this.emailType === 'remindEmployer') {
      payload['email_id'] = this.students[0].contact_01_primary_email;
    ...
    if (this.emailType === 'vacancy') {
      payload = { 
        email_id: this.students.map(vacancy => vacancy.company_info[0].contact_01_primary_email).join(),
        // category_id: this.emailForm.value.category_id,
    ...
    if (this.emailType === 'placement_company') {
      payload = { 
        email_id: this.students.map(company => company.email).join(),
        category_id: this.emailForm.value.category_id,
    ...
    if (this.emailType === 'users') {
      payload = { 
        email_id: this.students.map(student => student.email).join(),
        category_id: this.emailForm.value.category_id,
        template_id: this.emailForm.value.template_id,
        subject: this.emailForm.value.subject,
        message: this.emailForm.value.message,
