...
manageFieldVisibility(changedField: any, fields: any[]) {
  ...
  console.log("changedField: any, fields", changedField, fields)
  const logic =  Array.isArray(changedField.logic)
  ? changedField.logic[0] || {}   // if array, get first item
  : changedField.logic || {};
  if (!logic.field_name || !logic.is) return;
...
//   console.log("Recalculated end_process:", disableFound);
// }
...
//     const targetField = fields.find(f => f.name === logic.field_name);
//     const targetField2 = section?.component?.find(f => f.name === logic.hide_show_field) || null;

//     console.log("targetField (for value check)", targetField);
//     console.log("targetField2 (to hide/show)", targetField2);

//     if (!targetField || !targetField2) return;
...
//     if (passed) {
//       if (logic.action === 'show_field') {
//         targetField2.hidden = false;
//       } else if (logic.action === 'hide_field') {
//         targetField2.hidden = true;
//         targetField.hidden = false;
//         targetField2.elementData.value = null;
//       }
//     } else {
//       if (logic.action === 'show_field') {
//         targetField2.hidden = true;
...
//       }
//     }
//     console.log("hideShowField", targetField2, "targetField", targetField)
//   } else {
...
//     const hideShowField = logic.hide_show_field
//       ? fields.find(f => f.name === logic.hide_show_field)
//       : null;
//     if (!hideShowField) return;
...
//   // updated manageFieldVisibility: searches current step first, then all sections/pages
// manageFieldVisibilitymulti(changedField: any, fields: any[], allSections?: any[]) {
//   if (!changedField?.logic) return;

//   changedField.logic.forEach((logic: any) => {
//     // search in current step first
//     let targetField = fields.find(
//       f => f.elementData?.field_name === logic.hide_show_field
//     );

//     // if not found, search across steps (only if allSections provided)
//     if (!targetField && Array.isArray(allSections)) {
//       targetField = this.findAcrossSections(allSections, logic.hide_show_field);
//     }

//     if (targetField) {
//       const conditionMet = this.checkCondition(
//         changedField.elementData?.value,
//         logic,
//         targetField
//       );

//       // apply visibility
//       targetField.hidden = logic.action === 'hide_field'
//         ? conditionMet
//         : !conditionMet;
//     }
//   });
...
// private findAcrossSections(sections: any[], fieldName: string): any | null {
//   if (!sections) return null;

//   for (const section of sections) {
//     if (section.component) {
//       const found = section.component.find(
//         (f: any) => f.elementData?.field_name === fieldName
//       );
...
// checkFieldLogic(changedField: any, fields: any[]) {
//   console.log("changedField, fields", changedField, fields);
...
//     hideShowField = section?.component?.find(f => f.name === logic.hide_show_field) || null;
//   } else {
//     hideShowField = logic.field_name
//       ? fields.find(f => f.name === logic.field_name)
//       : null;
//   }

//   if (!hideShowField) return;

//   console.log("passed", passed, targetField, logic.action);

//   if (passed) {
//     switch (logic.action) {
//       case "show_message":
//         targetField.is_message = logic.message?true:false;
//         targetField.message = logic.message;
//         this.errorMessage = logic.message || "";
...
//          targetField.is_message = logic.message?true:false;
//         targetField.message = logic.message;
//         hideShowField.hidden = true;                // ✅ use target2 if multi
//         // hideShowField.elementData.value = null;
//         break;
...
//         targetField.is_message = !!logic.message;
//         targetField.message = logic.message;
//         hideShowField.hidden = false;               // ✅ use target2 if multi
//         break;
...
//     if (logic.action === "hide_field") {
//       hideShowField.hidden = false;
//       // hideShowField?.elementData?.required = false;
//        targetField.is_message = false;
//       this.disbaledBtnLogic = false;
//       this.stopnext = false;
//       targetField.message = "";
//     } else if (logic.action === "show_field") {
//       hideShowField.hidden = true;
//       // hideShowField.elementData.value = null;
//        targetField.is_message = false;
//       this.disbaledBtnLogic = false;
//       this.stopnext = false;
//       targetField.message = "";
//     } else if (["show_message", "block_submission"].includes(logic.action)) {
//       this.errorMessage = null;
//       targetField.is_message = false;
//       this.disbaledBtnLogic = false;
//       this.stopnext = false;
...
// "End Form Process & Show Message" rule can be overruled by meeting the conditional logic of other field
