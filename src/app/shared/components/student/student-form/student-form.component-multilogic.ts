...
export class StudentFormComponent implements OnInit {
  ...
  async getStudentTaskForm(formId) {
    this.service.getStudentFormById({ _id: formId, student_id: this.userDetail?._id, company_id: this.taskDetail?.companyDetails?._id, vacancy_type: "internship", placement_id: this.userDetail?.placement_id, }).subscribe(response => {
      ...
      console.log("  this.studentFormDetail", this.studentFormDetail, this.multiStepForm)
    });
  }
...
getValueInsideSingleBracket(input: unknown): string[] {
  ...
  onNextOrSubmit(data, stepper: MatStepper, type) {
   ...
   let fields = data?.component?data?.component:data;
    ...
    // if (this.studentFormDetail?.type === 'simple') {
    //      // --- First, check custom field logic for all fields ---
    ...
    //   const isRequired = field.hidden && field.elementData?.required;
    ...
    const isInvalid = fields.some(field => {
      ...
      // Skip if not editable, not required, or description
      if (!isEditable || !isRequired || isDescription || isHidden) return false;
      ...
      // Return true if required field has no value
      return !hasValue;
    });

    console.log("isInvalid", isInvalid, this.stopnext, this.disbaledBtnLogic, this.processEnd);
    // 
  ...
  onCheckboxChange(field) {
    ...
    if (field.elementData?.type === 'checkbox') {
      ...
      // else{
      //   field.elementData.required = true;
      // }


    }
    // if(this.studentFormDetail?.type === 'multi_step') {
    //      this.checkFieldLogic(field, this.multiStepForm);
    //   }else{
    //      this.checkFieldLogic(field, this.singleStepForm);
    //   }
  }

  // private checkCondition(fieldValue: any, rule: any, targetField: any): boolean {
  //   const value = rule.value;
  //   const fieldType = targetField?.elementData?.type;

  //     if (fieldValue == null || fieldValue === '') return false;

  //     // --- TEXT FIELDS: single-line or multi-line → compare LENGTH ---
  //     if (fieldType === 'single-line' || fieldType === 'multi-line') {
  //       const length = fieldValue?.toString()?.length || 0;
  //       switch (rule.is) {
  //         case 'eq':  return length == value;
  //         case 'neq': return length != value;
  //         case 'lt':  return length < value;
  //         case 'gt':  return length > value;
  //         case 'lte': return length <= value;
  //         case 'gte': return length >= value;
  //         default:    return true;
  //       }
  //     }

  //     // --- NUMBER FIELD → numeric comparison ---
  //     if (fieldType === 'number') {
  //       const numVal = parseFloat(fieldValue);
  //       const numRule = parseFloat(value);
  //       switch (rule.is) {
  //         case 'eq':  return numVal == numRule;
  //         case 'neq': return numVal != numRule;
  ...
  //     // --- DATE OR TIME FIELD → compare timestamps ---
  //     if (fieldType === 'date' || fieldType === 'time') {
  //       const fieldDate = new Date(fieldValue).getTime();
  //       const ruleDate = new Date(value).getTime();
  //       if (isNaN(fieldDate) || isNaN(ruleDate)) return false; // invalid date
  //       switch (rule.is) {
  ...
  //     // --- ARRAY FIELD → includes / not_includes ---
  //     if (Array.isArray(fieldValue)) {
  //       switch (rule.is) {
  //         case 'includes':     return fieldValue.includes(value);
  //         case 'not_includes': return !fieldValue.includes(value);
  //         default:             return true;
  //       }
  //     }

  //     // --- DEFAULT → simple equality for string / other types ---
  //     switch (rule.is) {
  ...
  checkFieldLogic(changedField: any, fields: any[]) {
    ...
    if (!fields || !changedField) return;

    // --- Handle process-end / stop-next states ---
    if (this.processEnd || this.stopnext) {
      if (this.processEndObj?.index === changedField.index) {
        ...
        console.log("processEnd check passed", passed);

        if (!passed) {
          this.resetFieldState(changedField);
        }
      }
      return;
    }

    let logicArray = changedField.logic || [];
    if (!logicArray.length) return;

    // --- Sort rules by condition priority: eq, lt, lte, gt, gte ---
    const priorityOrder = ["eq", "lt", "lte", "gt", "gte"];
    logicArray.sort((a, b) => priorityOrder.indexOf(a.is) - priorityOrder.indexOf(b.is));

    for (const rule of logicArray) {
      const targetField = fields.find(f => f.name === rule.field_name);
      ...
      const shouldCheck =
        ...
        fieldValue !== null &&
        fieldValue !== "" ||
        targetField.id === "checkbox";

      if (!shouldCheck) continue;

      const passed = this.checkCondition(fieldValue, rule, targetField);
      console.log("Rule result:", passed, rule, targetField);

      // Determine hide/show field
      let hideShowField = null;
      if (passed && rule.hide_show_field) {
        if (this.studentFormDetail?.type === 'multi_step') {
          const section = this.multiStepForm.find(el => el.name === rule.action_page_type);
          hideShowField = section?.component?.find(f => f.name === rule.hide_show_field) || null;
        } else {
          hideShowField = fields.find(f => f.name === rule.hide_show_field) || null;
        }
      }
      console.log("hideShowField", hideShowField, passed)

      // Apply actions if rule passed
      if (passed) {
        this.applyRuleAction(targetField, hideShowField, rule);
        // Stop processing further rules after first match
        break;
      } else {
        this.resetRuleAction(targetField, hideShowField, rule);
      }
    }
  ...
  // --- Helper to apply rule actions when passed ---
  private applyRuleAction(targetField: any, hideShowField: any, rule: any) {
    targetField.is_message = !!rule.message;
    targetField.message = rule.message || "";
    ...
    switch (rule.action) {
      ...
      case "block_submission":
        ...
        this.stopnext = true;
        this.errorMessage = rule.message || "Form submission blocked";
        break;
      ...
      case "end_process":
        ...
        this.errorMessage = rule.message || "Process ended";
        break;
    }
  }

  // --- Helper to reset rule actions when condition fails ---
  private resetRuleAction(targetField: any, hideShowField: any, rule: any) {
    switch (rule.action) {
      case "hide_field":
  ...
  // --- Helper to reset a field and global process states ---
  private resetFieldState(field: any) {
  ...
  //     const passed = field.elementData?.value?this.checkCondition(field.elementData?.value, logic, field):false;
  ...
  private recalculateEndProcess(fields: any[]) {
    ...
    for (const field of fields) {
      const logicArray = field.logic || [];
      if (!logicArray.length) continue;

      for (const rule of logicArray) {
        // Only consider end_process or block_submission rules
        if (!["end_process", "block_submission"].includes(rule.action)) continue;

        const fieldValue = field.elementData?.value;
        const passed = fieldValue ? this.checkCondition(fieldValue, rule, field) : false;

        console.log("Recalculate rule passed:", passed, "field:", field, "rule:", rule);

        if (passed) {
          disableFound = true;
          this.processEndObj = { index: field.index, action: rule.action };
          break; // stop at the first passing end_process/block_submission
        }
      }

      if (disableFound) break; // stop outer loop too
    }
  ...
  private checkCondition(fieldValue: any, rule: any, targetField: any): boolean {
    ...
    console.log("fieldValue: any, rule: any, targetField", fieldValue, "rule = = =", rule, " = = == = = = targetField = = = == ", targetField)
    const value = rule.value;
    const fieldType =
    ...
    if (fieldType === 'date') {
      const fieldDate = new Date(fieldValue).getTime();
      const ruleDate = new Date(value).getTime();
      if (isNaN(fieldDate) || isNaN(ruleDate)) return false;
      switch (rule.is) {
        case 'eq': return fieldDate === ruleDate;
        case 'neq': return fieldDate !== ruleDate;
        case 'lt': return fieldDate < ruleDate;
  ...
  manageFieldVisibility(changedField: any, fields: any[]) {
    const logicArray = changedField.logic || [];
    if (!logicArray.length) return;

    for (const rule of logicArray) {
      if (!rule.field_name || !rule.is) continue;

      let targetField: any;
      let hideShowField: any = null;

      // Determine target field and optional hide/show field
      if (this.studentFormDetail?.type === 'multi_step' && rule.hide_show_field) {
        const section = this.multiStepForm.find(el => el.name === rule.action_page_type);
        targetField = fields.find(f => f.name === rule.field_name);
        ...
        targetField = fields.find(f => f.name === rule.field_name);
        if(!targetField?.elementData?.value){
        hideShowField = rule.hide_show_field ? fields.find(f => f.name === rule.hide_show_field) : null;
        }
      }
      ...
      const passed = this.checkCondition(fieldValue, rule, targetField);
      console.log("manageFieldVisibility -> passed:", passed, rule, targetField);

      if (passed) {
        // --- Apply actions when condition passes ---
        targetField.is_message = !!rule.message;
        targetField.message = rule.message || "";
        ...
        // Stop processing further rules if a terminating rule passed
        if (["show_message", "block_submission", "end_process"].includes(rule.action)) break;

      } else {
        // --- Reset actions if condition fails ---
        switch (rule.action) {
          case "hide_field":
            if (hideShowField) {
              hideShowField.hidden = false;
