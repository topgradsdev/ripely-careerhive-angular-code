...
@Component({
  selector: 'app-report-incident-student-form',
  templateUrl: './report-incident-student-form.component.html',
  styleUrls: ['./report-incident-student-form.component.scss']
})
export class ReportIncidentStudentFormComponent implements OnInit {
  userDetail = null;
  taskDetail = null;
...
getValueInsideSingleBracket(input: unknown): string[] {
  ...
  submitWorkflowttachment(fields) {
    // this.goBack();
    const payload = {
      placement_id: this.params?.placement_id,
      student_id: this.userDetail?._id,
  ...
  backNavigaqte(){
     if(this.selectedTab.includes("ongoing")){
        this.router.navigate(['/student/my-placements'],  { queryParams: {stage: 'ongoing', placement_id: this?.placement_id, student_id: this.userDetail?._id, vacancy_id: this.params?.vacancy_id, id:this.params?.id , type:'ongoing'}  });
      }else if(this.selectedTab.includes("pre-placement")){
        this.router.navigate(['/student/my-placements'],  { queryParams: {stage: 'pre-placement', placement_id: this?.placement_id, student_id: this.userDetail?._id, vacancy_id: this.params?.vacancy_id, id:this.params?.id, type:'pre-placement'}  });
      }else if(this.selectedTab.includes("post-placement")){
        this.router.navigate(['/student/my-placements'],  { queryParams: {stage: 'post-placement', placement_id: this?.placement_id, student_id: this.userDetail?._id, vacancy_id: this.params?.vacancy_id, id:this.params?.id, type:'post-placement'}  });
