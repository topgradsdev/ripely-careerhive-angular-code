...
@Component({
  selector: 'app-report-incident-employer-form',
  templateUrl: './report-incident-employer-form.component.html',
  styleUrls: ['./report-incident-employer-form.component.scss']
})
export class ReportIncidentEmployerFormComponent implements OnInit {
  userDetail = null;
  taskDetail = null;
...
get visibleStepIndexes() {
  ...
  getForm(){
    ...
    this.service.getreportFormAuto(payload).subscribe(async response => {
      console.log("response", response)
      this.studentFormDetail = response.data;
...
getValueInsideSingleBracket(input: unknown): string[] {
  ...
  submitForm() {
   ...
   @ViewChild('successSubmit') successSubmit: ModalDirective;
  submitWorkflowttachment(fields) {
    const payload = {
      ...
      employer_name: this.userDetail?.first_name +' '+this.userDetail?.last_name,
      vacancy_type: this.studentNavParam?.vacancy_type=='project'?'project': "internship"
    }
    this.service.submitReportForm(payload).subscribe(res => {
      this.service.showMessage({
        message: res.msg?res.msg:"Incident report submitted successfully"
