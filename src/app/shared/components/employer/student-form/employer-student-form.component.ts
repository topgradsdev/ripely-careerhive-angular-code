...
@Component({
  selector: 'app-employer-student-form',
  templateUrl: './employer-student-form.component.html',
  styleUrls: ['./employer-student-form.component.scss']
})
export class EmployerStudentFormComponent implements OnInit {
  userDetail = null;
  taskDetail = null;
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      if (params.taskId) {
        ...
        this.tab = params.tab; 
        this.studentNavParam = JSON.parse(params?.studentNavParam);
        this.studentNavParam['vacancy_type'] = params.vacancy_type;
        this.getTaskDetail(params.taskId);

        console.log(" == = = = = == ",  this.studentNavParam?.vacancy_type=='project'?'project': "internship")
      }
    });
  }
  ...
  getTaskDetail(taskId) {
    ...
    this.service.getTaskDetail(payload).subscribe(async response => {
      ...
      }else if (this.taskDetail?.task_status === "completed") {
        this.getSubmittedTask();
        this.showDownload = true;
  ...
  async getStudentTaskForm(formId) {
    this.userDetail = JSON.parse(localStorage.getItem("userDetail"));
    this.service.getStudentFormById({ _id: formId, student_id: this.studentNavParam?.student_id, company_id: this.userDetail._id,  vacancy_type: this.studentNavParam?.vacancy_type=='project'?'project': "internship", placement_id: this.studentNavParam?.placement_id, }).subscribe(response => {
...
uploadFile(event, field) {
  ...
  get formDisplayStatus(): string {
    const form = this.taskDetail?.form_status?.toLowerCase();
    const emp = this.taskDetail?.employee_status?.toLowerCase();
    console.log("emp === 'completed'", emp === 'completed')
    if (emp === 'completed') {
      return 'Submitted';
    }
  ...
  submitWorkflowttachment(fields, status) {
    const payload = {
      ...
      vacancy_type: this.studentNavParam?.vacancy_type=='project'?'project': "internship"
    }
    this.service.submitWorkFlowForm(payload).subscribe(res => {
      ...
      if(this.tab=="completed"){
        // this.router.navigate(['/employer/completed']);
        this.goBack();
      }else{
  ...
  checkFieldPermission(permissions, data?) {
  ...
  deleteFile(index, form) {
    ...
    //   const onlyDescriptions = fields.every(field => field.elementData?.type === 'description');
    //   if (onlyDescriptions) {
    //     stepper.next();
    //     return;
    //   }
    
    //   // Validate only fields that are required, not description, and visible
    //   const isInvalid = fields.some(field => {
    //     const accessLevel = getAccessLevel(field.elementData?.permissions);
    //     const isVisible = accessLevel !== 'hidden';
    //     const isEditable = accessLevel === 'editable';
    ...
    //       ? field.elementData?.items.some(item => item.selected) // Check if any checkbox is selected
    //       : !!field.elementData?.value; // Check if any other field has a value
    
    //     return isVisible && isEditable && isRequired && !isDescription && !hasValue;
    //   });
...
private checkCondition(fieldValue: any, rule: any, targetField: any): boolean {
  ...
  if (fieldType === 'single-line' || fieldType === 'multi-line') {
    ...
    console.log("length < ruleNum", length < ruleNum, length , ruleNum)
    switch (rule.is) {
...
manageFieldVisibility(changedField: any, fields: any[]) {
  ...
  console.log("changedField", changedField, "fields", fields)
  const logic =  Array.isArray(changedField.logic)
  ? changedField.logic[0] || {}   // if array, get first item
  : changedField.logic || {};
  if (!logic.field_name || !logic.is) return;
  ...
  if (this.studentFormDetail?.type === 'multi_step' && logic.hide_show_field) {
    ...
    if(!targetField?.elementData?.value || targetField?.elementData?.value == "Nah"){
      hideShowField = section?.component?.find(f => f.name === logic.hide_show_field) || null;
      console.log("hideShowField", hideShowField);
    }
  } else {
    targetField = fields.find(f => f.name === logic.field_name);
   if(!targetField?.elementData?.value || targetField?.elementData?.value == "Nah"){
  ...
  if (passed) {
    switch (logic.action) {
      case 'show_field':
        ...
        // if (hideShowField.elementData) {
        //   hideShowField.elementData.is_required = false;
        // }
...
getOpacity(transparency: number): number {
  if (!transparency && transparency !== 0) return 1; 
