...
@Component({
  selector: 'app-view-company-departments',
  templateUrl: './view-company-departments.component.html',
  styleUrls: ['./view-company-departments.component.scss']
})
export class ViewCompanyDepartmentsComponent implements OnInit {
  placementId: any;
  ...
  getDeprtmentbyCompany() {
    this.service.getDeprtmentbyCompany({company_id:this.employerProfile._id}).subscribe((res:any) => {
    
      if (res.status == HttpResponseCode.SUCCESS) {
        this.deprtmentList = res.data.departments;
        if(this.selectedDepartment && this.selectedDepartment.company_department_id){
            if(this.selectedDepartment){
              let find =  this.deprtmentList.find(el=>el.company_department_id==this.selectedDepartment.company_department_id);
