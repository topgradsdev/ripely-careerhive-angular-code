...
@Component({
  selector: 'app-employer-placement-details',
  templateUrl: './employer-placement-details.component.html',
  styleUrls: ['./employer-placement-details.component.scss']
})
export class EmployerPlacementDetailsComponent implements OnInit {
  ...
  taskStatus(status, item) {
    // console.log("item", item);
    // if(item && item.completed_step_count == 0){
    //   return 'Pending';
    // }
    const taskStatus = {
      lock: "Locked",
...
//   if (item && item.employee_status !== 'completed' && item.form_status === 'save') {
//     return 'In Progress';
//   }
...
// }
  ...
  goToStudentForm(task) {
    ...
    // this.router.navigate(['/employer/projects/employer-student-form'], { queryParams: { taskId: task._id, studentNavParam: JSON.stringify(this.studentNavParam), tab:'ongoing', vacancy_type:"internship" } });
  }
uploadFile(event: Event, task: any) {
  ...
  for (const file of filesArray) {
    if (file.size > 5000971) { // ~5 MB
      ...
      input.value = ''; // Clear input if validation fails
      return;
    }
    ...
    this.service.uploadOthersMedia(formData).subscribe((resp: any) => {
       task.file = resp;  // Push each response to the file array
    });
  }
 ...
 getInternshipDuration(start: string, end: string): string {
  if(start && end){
   ...
   gotoReportIncident(data){
    ...
    this.router.navigate(['/employer/ongoing/report-incident-student-form'], { queryParams: { stage: this.studentNavParam.type,placement_id:this.studentNavParam.placement_id, student_id: this.studentNavParam?.student_id, vacancy_id:this.studentNavParam?.vacancy_id, company_id: this.userDetail?._id, type:'internship'}  });
  }
  ...
  //  this.router.navigate(['/employer/ongoing/report-incident-student-form'], { queryParams: { stage: 'ongoing',placement_id: this.studentNavParam.type,placement_id, student_id: this.studentNavParam?.student_id, vacancy_id:this.studentNavParam?.vacancy_id, company_id: this.userDetail?._id, type:'internship'}  });
}
