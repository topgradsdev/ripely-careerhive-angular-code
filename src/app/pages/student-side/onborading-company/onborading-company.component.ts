...
@Component({
  selector: 'app-onborading-company',
  templateUrl: './onborading-company.component.html',
  styleUrls: ['./onborading-company.component.scss'],
  providers: [BsModalRef]
})
export class OnboradingCompanyComponent implements OnInit {
  @ViewChild('successCompanyModal') public successCompanyModal: ModalDirective;
  @ViewChild('alreadyCompanyModal') public alreadyCompanyModal: ModalDirective;
  ...
  ngOnInit(): void {
    ...
    // this.essentials.get('abn_acn').disable();
  ...
  getName(id) {
    const dept = this.departmentList.find(el => el._id == id);
 ...
 addSpacesInABN() {
  ...
  updateStatus(){
    this.service.updateSelfSourceStatus({ student_id:this.userDetail?._id, self_sourced_status:"company_already_exists"}).subscribe(res => {
      if (res.status == 200) {
        this.alreadyCompanyModal.hide();
        this.location.back();
      } else {
  ...
  async postVacancy() {
    ...
    this.service.compnayOnboardingSelfSource(body).subscribe(res => {
      if (res.status == 200) {
        this.service.showMessage({
...
// }
  ...
  fun(stepper: MatStepper) {
    if (this.essentials.invalid) {
      ...
      let body = {
         abn_acn:this.essentials.value.abn_acn, company_name:this.essentials.value.company_name, address:this.essentials.value.address, state:this.essentials.value.state, postal_code:this.essentials.value.postal_code
      }
      this.service.validateSelfSourcedCompany(body).subscribe(res => {
        console.log(res);
        if(res.code==200){
          if(res.result && res.result.length>0){
            this.alreadyCompanyModal.show();
          }else{
