...
@Component({
  selector: 'app-pre-placement-tab',
  templateUrl: './pre-placement-tab.component.html',
  styleUrls: ['./pre-placement-tab.component.scss']
})
export class PrePlacementTabComponent implements OnInit {
...
// }
  ...
  goToStudentForm(task) {
    this.router.navigate(['/student/student-form'], { queryParams: { taskId: task._id , stage:'pre-placement'} });
  }

  goToStudentVideo(task) {
    this.router.navigate(['/student/student-video'], { queryParams: { taskId: task._id , stage:'pre-placement'} });
  }
...
uploadFile(event: Event, task: any) {
  ...
  task.file = []; // Initialize as an array
  ...
  for (const file of filesArray) {
    if (file.size > 5000971) { // 5 MB
      ...
      input.value = ''; // Clear input to allow reselect
      return;
    }
    ...
    this.service.uploadOthersMedia(formData).subscribe((resp: any) => {
       task.file = resp; // Push response into array
    });
  }
  ...
  callUpdate(){
    // console.log("this.studentProfile", this.studentProfile)
        this.service.updateSelfSourceStatus({ student_id:this.userProfile?._id, self_sourced_status:"done"}).subscribe(res => {
        // this.service.showMessage({
        //   message: "Task submitted successfully"
        // });
        this.studentProfile.self_sourced_status = 'done';
        this.getWorkFlowTask();
      }, err => {
        this.service.showMessage({
          message: err.error.errors ? err.error.errors.msg : 'Something went Wrong'
