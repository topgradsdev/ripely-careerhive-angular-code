...
@Component({
  selector: 'app-post-placement-tab',
  templateUrl: './post-placement-tab.component.html',
  styleUrls: ['./post-placement-tab.component.scss']
})
export class PostPlacementTabComponent implements OnInit {
...
// }
  ...
  goToStudentForm(task) {
    this.router.navigate(['/student/student-form'], { queryParams: {taskId: task._id,stage:'post-placement' }});
  }
 ...
 uploadFile(event: Event, task: any) {
  ...
  // Initialize task.file as an array to collect responses
  task.file = [];
  ...
  for (const file of filesArray) {
    ...
    this.service.uploadOthersMedia(formData).subscribe((resp: any) => {
       task.file = resp;  // Add response to the file array
    });
  }
