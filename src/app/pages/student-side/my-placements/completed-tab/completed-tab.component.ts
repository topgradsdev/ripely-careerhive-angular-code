...
@Component({
  ...
  styleUrls: ['./completed-tab.component.scss']
})
...
// }
  ...
  goToStudentForm(task) {
    this.router.navigate(['/student/student-form'], { queryParams: {taskId: task._id, stage:'completed' }});
  }
...
uploadFile(event: Event, task: any) {
  ...
  for (const file of filesArray) {
    ...
    this.service.uploadOthersMedia(formData).subscribe((resp: any) => {
      task.file = resp;  // Add each file response to the task.file array
    });
  }
...
parseDate(dateStr: string | null | undefined): Date | null {
 ...
 getInternshipDuration(start: string, end: string): string {
  ...
  gotoReportIncident(data){
    ...
    this.router.navigate(['/student/report-incident-student-form'], { queryParams: { stage: 'completed',placement_id: this.userProfile?.placement_id, student_id: this.userProfile?._id, vacancy_id:this.userProfile?.company_allocation[0]?.vacancy_id, company_id: this.userProfile?.company_allocation[0]?.company_id, type:'intership'}  });
  }
