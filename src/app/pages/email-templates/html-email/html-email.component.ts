...
export class HtmlEmailComponent implements OnInit {
  ...
  //         const componentDetails = this.getComponentDetailsById(widget.data.id);
  ...
  //           m:componentDetails?.groupIndex,
  //           i:componentDetails?.itemIndex,
  //           actionType: "drop",
    ...
    if (this.selectedTemplate.widgets?.values?.length > 0) {
      this.droppedEmailTemplateItems = this.selectedTemplate.widgets.values.map((widget, i) => {
        ...
        const processItem = (item) => {
          const details = this.getComponentDetailsById(item.id);
          return {
            instance: {
              data: {
                ...item,
                component: details?.component,
                m: details?.groupIndex,
                i: details?.itemIndex
              }
            }
          };
        ...
        const blankSections = widget.item.blank?.[0] ? [processItem(widget.item.blank[0])] : [];
        const twoFirst = widget.item.two?.first?.[0] ? [processItem(widget.item.two.first[0])] : [];
        const twoSecond = widget.item.two?.second?.[0] ? [processItem(widget.item.two.second[0])] : [];
        const threeFirst = widget.item.three?.first?.[0] ? [processItem(widget.item.three.first[0])] : [];
        const threeSecond = widget.item.three?.second?.[0] ? [processItem(widget.item.three.second[0])] : [];
        const threeThird = widget.item.three?.third?.[0] ? [processItem(widget.item.three.third[0])] : [];
        ...
        const rebuiltWidget = {
          ...
          i: componentDetails?.itemIndex,
          actionType: 'drop',
          elementData: widget.data?.elementData ?? { insideSection: false },
          droppedSectionItems: {
            blankSections,
            twoSections: {
              firstSection: twoFirst,
              secondSection: twoSecond
            },
            threeSections: {
              firstSection: threeFirst,
              secondSection: threeSecond,
              thirdSection: threeThird
            }
          }
        };

        return { instance: { data: rebuiltWidget } };
      });
...
getComponentDetailsById(id: string): { component: any, groupIndex: number, itemIndex: number } | null {
  for (let groupIndex = 0; groupIndex < this.draggableEmailTemplateElements.length; groupIndex++) {
    const group = this.draggableEmailTemplateElements[groupIndex];
    const itemIndex = group.items.findIndex(item => item.id === id);
    if (itemIndex !== -1) {
      return {
        component: group.items[itemIndex].component,
        groupIndex,
        itemIndex
      };
    }
  ...
  onEmailTemplateElementDrop(e: any, type?) {
    console.log("e", e);
    let dragged = e?.data;
   
    console.log("dragged", dragged);
   let component:any = dragged && dragged.data && dragged.data.component?dragged.data.component :this.draggableEmailTemplateElements[dragged.m].items[dragged.i].component;
  ...
  sortElement() {
    ...
    items.forEach(item => {
      // const obj = Object.assign({}, { data: item.instance.data });
      // this.onEmailTemplateElementDrop(obj, 'sort');
       let obj = { data: item.instance.data };
        ...
        console.log("obj", obj)
      // return false;
      this.onEmailTemplateElementDrop({data:obj}, 'sort');
    });
    this.reorderIndexing();
  }
