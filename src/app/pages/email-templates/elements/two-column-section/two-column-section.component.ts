 ...
 ngOnInit(): void {
  setTimeout(() => {
    if (this.data?.actionType === 'sort') {
      const twoSections = this.data?.droppedSectionItems?.twoSections;

      // Handle first section
      const firstInstance = twoSections?.firstSection?.[0]?.instance;
      if (firstInstance) {
        const obj = {
        ...
        // Clear first section before sort-drop
      ...
      const secondInstance = twoSections?.secondSection?.[0]?.instance;
      if (secondInstance) {
        const obj = {
        ...
        // Clear second section before sort-drop
