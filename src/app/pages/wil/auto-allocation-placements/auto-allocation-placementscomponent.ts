...
import { Component, ViewChild, ViewChildren, QueryList, ChangeDetectorRef, OnInit, ElementRef } from '@angular/core';
...
@Component({
  selector: 'app-auto-allocation-placements',
  templateUrl: './auto-allocation-placements.component.html',
  styleUrls: ['./auto-allocation-placements.component.scss'], animations: [
...
export class PlacementsTabComponent implements OnInit {
  ...
  innerDisplayedColumns1 = ['first_name', 'Preference', 'skill', 'status'];
...
// allocations
  ...
  searchStudents() {
    let payload = {
      ...
      placement_id: this.id,
       is_archive: false
      // is_archive: this.selectedIndex === 1 ? true : false
    }
...
async checkPlaced(): Promise<boolean> {
  ...
  chooseValue(){
    ...
    callAutoMatchApi(){
      ...
      this.service.autoAllocateStudents(body).subscribe(res => {
        if (res.status == HttpResponseCode.SUCCESS) {
    ...
    submitAutoMatch(){
      let body = {
        allocation_data:this.allocations,
        placement_id:this.id
      };
