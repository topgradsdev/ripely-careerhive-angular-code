...
@Component({
  ...
  styleUrls: ['./create-project-admin.component.scss']
})
export class CreateProjectAdminComponent implements OnInit {
  ...
  field2: any;
  field3: any;
@ViewChild('ProjectCreated', { static: false }) ProjectCreated: ModalDirective;     
  jobSkills: any = [];
  ...
  constructor(private fb: FormBuilder,
    ...
    private activatedRoute: ActivatedRoute, private router: Router, private fileIconService: FileIconService) { }
  userDetail:any = {};
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (params.company_id) {
        ...
        this.getChildCompany(this.selectedCompanyId);
  ...
  getComapany(id){
    if(id){
        console.log("this.childCompanyList.find(el=>el._id == id).company_name", id, this.childCompanyList, this.childCompanyList.find(el=>el._id == id))
        return this.childCompanyList.length>0? this.childCompanyList.find(el=>el._id == id).company_name:'';
    }
  }
  getVacancyById() {
    const obj = {
    ...
    this.service.getVacancyById(obj).subscribe((res: any) => {
      ...
      this.editorForm.patchValue({
        ...
        write_my_own_description: result?.write_my_own_description?result?.write_my_own_description:result?.job_description,
      ...
      console.log("this.editorForm", this.editorForm.value)
      if(result?.no_limit){
        this.toggleNoLimit();
      }
  ...
  getSkill(skills) {
    ...
    this.service.getJobSkills(obj).subscribe(res => {
      ...
      this.selectedJobPreferredSkills = skills.map(skill => {
        const found = res.data.find(rSkill => rSkill._id === skill);
        if (found) {
          skill = found;
        }
  ...
  postVacancy() {
    ...
    const obj = {
      ...
      company_id: this.essentials.value.company_id,
      ...
      is_pending: this.selectedVacancy?.is_pending ?? false,
  ...
  fun(stepper: MatStepper) {  
    ...
    // skillsevent1:any = [];
    editorFormgetMultipleSkillsByIds1(ids?: any) {
      this.skillsevent1 = this.jobPreferredSkills.filter((skill: any) => {
        return this.selectedJobPreferredSkills.findIndex(sk => sk._id === skill._id) !== -1;
      });
  }
