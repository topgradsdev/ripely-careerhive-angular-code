...
@Component({
  selector: 'app-placement-workflow-project',
  templateUrl: './placement-workflow-project.component.html',
  styleUrls: ['./placement-workflow-project.component.scss']
})
export class PlacementWorkflowProjectComponent implements OnInit {
  placementId: any;
  modules = {
    toolbar: {
      container: [
 ...
 async showOnly(data){
  ...
  // }
  addStep() {
    const workflowType = this.placementTypes.find(placementType => placementType.selected);
    if (!workflowType) {
      console.warn('No selected placementType. Make sure a tab is selected.');
      return;
    }
    this.selectedWorkflowType = workflowType;
    let addNewStep = {
    ...
    this.service.addNewStep(addNewStep).subscribe((response: any) => {
      if (response.status === HttpResponseCode.SUCCESS) {
        addNewStep['_id'] = response?.step_id;
        this.workFlowTask.push(addNewStep);
        console.log(this.workFlowTask, "this.workFlowTask", addNewStep)
      }
    });
  }
  ...
  // async drop(event: CdkDragDrop<string[]>, steps) {
  ...
  sendEmail() {
    ...
    this.service.sendReminderEmail(payload).subscribe((res: any) => {
      ...
      if (res.status == HttpResponseCode.SUCCESS) {
        ...
        this.service.showMessage({
          message: res.msg ? res.msg : 'Email not sent, please add student'
        });
      }
    }, err => {
      this.service.showMessage({
  ...
  chooseValue(){
   ...
   selectedKey:any = '';
    ...
    //     return title.includes(this.selectedKey);
    //   });
    // }
  ...
  createDocFolder() {
    ...
    this.service.createDocFolder({ placement_id: this.placementId, name: folderName }).subscribe((res: any) => {
      if (res.status == HttpResponseCode.SUCCESS) {
        this.docLibraryFolders.push({ _id: res.data._id, name: folderName, files: [], expanded: true });
      }
    });
  }
