...
// })
// export class CompanyDetail111Component {
...
//           } else {
//             // this.studentFormDetail?.fields = [this.studentFormDetail?.fields[0]]
//             this.multiStepForm = this.studentFormDetail?.fields;
//             // this.multiStepForm = [this.studentFormDetail?.fields[0]];
//           }
//           const self = this;
//           setTimeout(() => {
...
//     getValueInsideSingleBracket(input) {
//       // Check if `input` is defined and is either an array or string
//       if (!input) {
//         return [];
//       }
...
//            const hiddenSteps = element.querySelectorAll('.mat-vertical-content[aria-hidden="true"]');
//           hiddenSteps.forEach(el => (el as HTMLElement).style.display = 'block');
    
//           try {
//             // const canvas = await domtoimage.toPng(element, { quality: 1 , cacheBust: true});
//             const canvas = await domtoimage.toPng(element, { quality: 1,bgcolor: '#ffffff',
//                 // cacheBust: true,
//                 // useCORS: true,
//                 // imagePlaceholder: ''
//               });
...
// //             let position = 0;
// //   console.log("pdf", pdf, imgData, 'PNG', 0, position, imgWidth, imgHeight)
...
//         company_id: this.companyId
//       }
//       this.service.getActivityLogs(payload).subscribe(res => {
//         if (res.status == HttpResponseCode.SUCCESS) {
//           this.activityLogs = res.result;
  
//           // this.activityLogs = res.result.reduce((acc, item) => {
//           //   // Use created_at as the key
