...
export class ViewCompanyDetailsComponent implements OnInit {
  ...
  @ViewChild('setheadquarter') setheadquarter;
  @ViewChild('setheadquartersuccess') setheadquartersuccess;
  @ViewChild('removeheadquarter') removeheadquarter;
  @ViewChild('removeheadquartersuccess') removeheadquartersuccess;
  @ViewChild('addChildCompanyModle') addChildCompanyModle: ModalDirective;
  @ViewChild('addChildCompanySuccess') addChildCompanySuccess: ModalDirective;
  @ViewChild('removechild') removechild: ModalDirective;
  @ViewChild('removeChildwithVacancyConfirm') removeChildwithVacancyConfirm: ModalDirective;
  @ViewChild('removeChildwithVacancyProcess') removeChildwithVacancyProcess: ModalDirective;
  @ViewChild('removeChildwithVacancySuccess') removeChildwithVacancySuccess: ModalDirective;
  @ViewChild('removeHeadQuaterwithVacancyConfirm') removeHeadQuaterwithVacancyConfirm: ModalDirective;
  @ViewChild('removeHeadQuaterwithVacancyProcess') removeHeadQuaterwithVacancyProcess: ModalDirective;
  ...
  addChildCompanySubmit(){
     let body ={ parent_id:this.addChildCompanyForm.value.parent_company_id, company_id:this.employerProfile._id, is_child:true }
      this.service.addChildCompanyBYHQ(body).subscribe((res:any) => {
          if (res.code == 200) {
          this.addChildCompanyModle.hide();
  ...
  addCompanyHeadQuarter(){
       const body = {
      company_id: this.companyId,
      "is_headquarter": true
    }
    this.service.setAsHeadquarter(body).subscribe((response: any) => {
      this.service.showMessage({
          message: "Company Headquarters have been updated."
      });
      this.getEmployerProfile(this.documentpage);
       this.setheadquarter.hide();
      this.setheadquartersuccess.show();
    }, (err)=>{
      this.service.showMessage({
        message: err.error.errors ? err.error.errors.msg : 'Something went Wrong'
      });
  ...
  checkExistdata(){
    ...
    this.service.checkchildCompanyVacancy(body).subscribe((response: any) => {
      if(response.code === 200){
        this.existChild = response;
        if(response.is_child_vacancy){
          this.removeHeadQuaterwithVacancyConfirm.show();
        }else{
          this.removeheadquarter.show();
        }
      }
    }, (err)=>{
      this.service.showMessage({
  ...
  removeCompanyHeadQuarter(){
      const body = { company_id:this.employerProfile._id, is_child_company: this.existChild.is_child_company, is_child_vacancy:this.existChild.is_child_vacancy, vacancy_data:this.existChild.result }
    this.service.removeHeadquarter(body).subscribe((response: any) => {
      if(response.code === 200){
        this.removeheadquarter.hide();
        this.removeheadquartersuccess.show();
        this.employerProfile.is_headquarter = false;
        if(this.existChild.result && this.existChild.result.length>0){
           this.removeChildwithVacancySuccess.show();
        } 
        this.existChild = null;
      }
  ...
  removechildCompany(){
      this.service.removeChildCompany({company_id:this.employerProfile._id}).subscribe((res:any) => {
        if (res.code == 200) {
        this.service.showMessage({ message: "Successfully detch company" });
        this.removechild.hide()
        this.employerProfile.is_child = false;
  ...
  ngOnInit(): void {
    ...
    this.addChildCompanyForm = new FormGroup({
      parent_company_id: new FormControl('', [Validators.required]),
    })
  ...
  companyProfileEdit() {
    this.employerProfile.description = this.employerProfile?.description?this.employerProfile?.description:this.employerProfile?.description
  ...
  getEmployerProfile(page) {
    ...
    this.service.getEmployerProfile(payload).subscribe(response => {
      ...
      this.form.patchValue({
        ...
        description: this.employerProfile && this.employerProfile.description ? this.employerProfile.description : "",
...
getFilDoc(event: Event) {
  ...
  goBack() {
  let check =  sessionStorage.getItem('r_url');
 ...
 getValueInsideSingleBracket(input: unknown): string[] {
  ...
  async submitWorkflowttachment(fields) {
    ...
    const payload =await  {
      ...
      // "valid_to": new Date(new Date().setDate(new Date().getDate() + 365)).toISOString(),
      ...
      "valid_to": this.hcaafPending[0].valid_to
        ...
        : new Date(
            new Date().setDate(
              new Date().getDate() +
              (this.hcaafPending[0].hcaaf_validation
...
onPrevPage() {
  ...
  limit = 10;          // number of records per load
    ...
    onScrollDown() {
      ...
      if (this.isLoading || this.activityLogs.length >= this.activitLimit) return;
    ...
    getActivityLogs() {
      ...
      //         this.activityLogs = res.result;
      //       } else {
      //         this.activityLogs = [...this.activityLogs, ...res.result];
      //       }
      //       this.activitLimit = res.total || this.activitLimit;
      //     }
      ...
      //     this.service.showMessage({
      //       message:
      //         err.error.errors?.msg || 'Something went wrong while loading activities',
      //     });
      //   },
      // });
    }
  ...
  gotoCompanyStudentIncidentsList(data, type){
    this.router.navigate(['/admin/students/incident-reporting-list'], {queryParams: {company_id: data._id,type:type}});
  }
