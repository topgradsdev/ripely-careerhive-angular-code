...
@Component({
  selector: 'app-view-student-completed-placement',
  templateUrl: './view-student-completed-placement.component.html',
  styleUrls: ['./view-student-completed-placement.component.scss']
})
export class ViewStudentCompletedPlacementComponent implements OnInit {
  ...
  getOngoingStudentVacancyDetail() {
    console.log(" this.userProfile this.userProfile this.userProfile",  this.userProfile);
    const payload = {
    ...
    console.log("payloadpayloadpayload completed", payload);
    this.service.getOngoingStudentVacancyDetail(payload).subscribe((response: any) => {
     console.log("responseresponse", response)
      ...
      console.log("this.studentVacancyDetail",  this.studentVacancyDetail)
    });
  }
 ...
 getInternshipDuration(start: string, end: string): string {
  ...
  getWorkFlowTask() {
    ...
    this.service.getWorkFlowTaskStudentOngoing(payload).subscribe((response: any) => {
      ...
      // });
        this.completedPlacementWorkFlow.forEach(placement => {
...
uploadFile(event: Event, task: any) {
  ...
  task.file = []; // ✅ Initialize as array to store multiple uploads
  ...
  for (const file of filesArray) {
    ...
    this.service.uploadOthersMedia(formData).subscribe((resp: any) => {
      task.file = resp;  // ✅ Add each file response to array
    });
  }

  input.value = ''; // ✅ Clear input after all files processed
}
