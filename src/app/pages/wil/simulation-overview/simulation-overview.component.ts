...
export class SimulationOverviewComponent implements OnInit {
  modules = {
    ...
    //   this.onChangePublishOn({ value: "Publish Instantly" });
      ...
      this.preEnrolmentForm = new FormGroup({
        hero_text: new FormControl("", Validators.required),
        subheading: new FormControl("", Validators.required),
        overview: new FormControl(""),
        proposition: new FormControl(""),
        proposition_content: new FormControl(""),
    ...
    // Pre-Enrolment
    preEnrolmentForm: FormGroup;
    showFilesExpanded = false;
    propositionContentExpanded = false;
    companiesList: any[] = [];
    isPreEnrolmentSaved = false;
    ...
    onSavePreEnrolment() {
      if (this.preEnrolmentForm.valid) {
        const payload = {
          placement_id: this.id,
          pre_enrolment: this.preEnrolmentForm.value,
          is_update: true,
        };
        this.service.editPlacementGroup(payload).subscribe((response: any) => {
          if (response.status == HttpResponseCode.SUCCESS) {
            this.isPreEnrolmentSaved = true;
            this.getPlacementOverviewDetails();
            this.service.showMessage({ message: response.msg });
          }
        });
      } else {
        this.preEnrolmentForm.markAllAsTouched();
      }
    }

    checkPreEnrolmentFieldInvalid(field: string): boolean {
      return (
        this.preEnrolmentForm.get(field)?.invalid &&
        (this.preEnrolmentForm.get(field)?.dirty ||
          this.preEnrolmentForm.get(field)?.touched)
      );
    }

    loadPreEnrolmentData() {
      if (this.placementGroupDetails) {
        const preEnrolment = this.placementGroupDetails['pre_enrolment'] || {};
        this.isPreEnrolmentSaved = !!(preEnrolment.hero_text || preEnrolment.subheading);
        this.preEnrolmentForm.patchValue({
          hero_text: preEnrolment['hero_text'] || '',
          subheading: preEnrolment['subheading'] || '',
          overview: preEnrolment['overview'] || '',
          proposition: preEnrolment['proposition'] || '',
          proposition_content: preEnrolment['proposition_content'] || '',
          offer_letter: preEnrolment['offer_letter'] || '',
        });
        this.companiesList = this.placementGroupDetails['companies'] || [];
      }
    }
