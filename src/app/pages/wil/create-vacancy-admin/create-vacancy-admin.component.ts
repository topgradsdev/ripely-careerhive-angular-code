...
@Component({
  selector: 'app-create-vacancy-admin',
  templateUrl: './create-vacancy-admin.component.html',
  styleUrls: ['./create-vacancy-admin.component.scss']
})
export class CreateVacancyAdminComponent implements OnInit {
  modules = {
    toolbar: [
...
redirectType:any = null
  ngOnInit(): void {
    ...
    this.essentials = this.fb.group({
      ...
      'subsidiary_vacancy': [false, [Validators.nullValidator]],
    ...
    this.editorForm = this.fb.group({
     ...
     this.routeForm = this.fb.group({
      'application_receiver': [null, [Validators.required]],
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (params.company_id) {
        this.selectedCompanyId = params.company_id;
  ...
  return a && b ? a === b || a._id === b._id : a === b;
};
            ...
            if (res.status == HttpResponseCode.SUCCESS) {
              ...
              setTimeout(() => {
                ...
                this.routeForm.patchValue({
                  application_receiver: this.contactList.filter(c =>
                    this.selectedVacancy.application_receiver.includes(String(c._id))
                  )
                  
                });
                // console.log("this.routeForm", this.routeForm.value, this.contactList);
              }, 500);
            } else {
                this.contactList = [];
            }
  ...
  getChildCompany(companyId: string, selectedParentCompanyId?: string) {
    this.service.getChildCompany({ company_id: companyId }).subscribe((res: any) => {
      if (res.status == HttpResponseCode.SUCCESS) {
        ...
        setTimeout(() => {
            console.log("caling", selectedParentCompanyId)
  ...
  getEmployerProfile() {
    ...
    this.service.getEmployerProfile(payload).subscribe(response => {
      ...
      this.getContactList(this.selectedCompanyId);
      //  if(this.employerProfile?.parent_id){
      //   this.essentials.patchValue({
      //     company_id:this.employerProfile?._id,
      //     parent_company_id: this.employerProfile?.parent_id,
      //     company_logo: this.employerProfile?.company_logo,
      //     companyName: this.employerProfile?.parent_data?.company_name,
      //    })
      //   this.getChildCompany(this.employerProfile?.parent_id, this.employerProfile?._id);   
      // }else{
       console.log("else")
        //  this.getChildCompany(this.employerProfile?.company_id);
         this.getChildCompany(this.employerProfile?._id);
         this.essentials.patchValue({
          companyName: this.employerProfile?.company_name,
          company_id: this.employerProfile?._id,
          company_logo: this.employerProfile?.company_logo,
         });
      // }
        this.getCompanyContactList(this.selectedCompanyId);
    });
  }
  ...
  getVacancyById() {
    ...
    this.service.getVacancyById(obj).subscribe((res: any) => {
      ...
      this.essentials.patchValue({
        companyTitle: this.selectedVacancy?.job_title,
        industry: this.selectedVacancy?.industry_id,
      ...
      this.selectedCompanyId = result?.company_id;
      // this.getContactList(result?.company_id);
  ...
  onCompanyChange(selected: any) {
   ...
   getComapany(id){
    if(this.childCompanyList && this.childCompanyList.length>0){
  ...
  postVacancy() {
    // console.log("this.essentials.value", this.essentials.value)
    ...
    if(this.essentials.value.parent_company_id){
      ...
      obj['company_name']= this.childCompanySelected && this.childCompanySelected.companyName?this.childCompanySelected.companyName:undefined;
      obj['company_logo'] = this.childCompanySelected && this.childCompanySelected.company_logo?this.childCompanySelected.company_logo:undefined;
    }else{
  ...
  getCompanyList(event?: any) {
    ...
    if (event.target.value) {
      ...
      const payload = {
        search: event && event.target && event.target.value?event.target.value:this.searchKey
      }
      this.service.getCompanyList(payload).subscribe(async(res) => {
        ...
        this.companyList = await this.companyList.filter(company => 
          company.company_name.toLowerCase().includes(this.searchKey.toLowerCase())
        );
  ...
  setCompanyLogo(company: any) {
    ...
    this.getChildCompany(company._id);
    this.getContactList(company._id);
    this.getCompanyContactList(company._id);
  }
  ...
  jobTitleList() {
    ...
    this.service.getJobTitle(param).subscribe(res => {
      ...
      //   this.variables.push(el.name);
      //   this.variables.push(el.code);
      // })
    })
    ...
    navigateToProjectTab() {
     ...
     if (redirectData || this.redirectType) {
        ...
        this.router.navigate(['/admin/wil/view-company-details'], {
          queryParams: {
            company_id: parsed?.company_id ?? this.selectedCompanyId,
            tab: parsed?.tab ?? 'Vacancies'
          }
        });
      } else {
